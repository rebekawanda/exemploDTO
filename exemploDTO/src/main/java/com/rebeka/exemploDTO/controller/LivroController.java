package com.rebeka.exemploDTO.controller;
...
import com.rebeka.exemploDTO.service.LivroService;
...
public class LivroController {
	...
	@PostMapping
	public ResponseEntity<LivroDTO> criar(@RequestBody @Validated Livro livro) {
		LivroDTO  salvarLivro = livroService.salvar(livro);
		return ResponseEntity.status(HttpStatus.CREATED).body(salvarLivro);
	}
	
	@PutMapping("/(id)")
	public ResponseEntity<LivroDTO> alteraClienteControl(@PathVariable Long id, @RequestBody @Validated Livro livro) {
